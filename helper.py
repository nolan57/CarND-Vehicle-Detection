...
def load_data(folder_path='./data'):
    ...
    images_nv = glob.glob(folder_path + '/non-vehicles/**/*.png')
    images_v = glob.glob(folder_path + '/vehicles/**/*.png')
    cars = []
    notcars = []
...
def train_test_svm(car_features, notcar_features):
    ...
    return svc, X_scaler, acc


def extract_and_train(cars, notcars, colorspace='RGB', orient=9, pix_per_cell=8,
    ...
    svc, X_scaler, acc = train_test_svm(car_features, notcar_features
                   )
    return svc, X_scaler, acc
...
def find_cars(img, ystart, ystop, scale, colorspace, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins):
    ...
    #if ctrans_tosearch is None:
    #    print('ctrans_tosearch is NONE!')
...
def draw_labeled_bboxes(img, heatmap):
    ...
    for car_number in range(1, labels[1] + 1):
        ...
        #img = cv2.putText(img, 'CAR: '+str(car_number), (bbox[0][0]+2,bbox[0][1]), 0, 1, (0,0,255), 2, cv2.LINE_AA)
...
def mult_scale_windows(topleftyx, overlap):
    ...
    for toplefty in topleftyx.keys():
        ...
        for toplefx in topleftyx[toplefty]:
            ...
            windows.append(((toplefx, toplefty), (buttomrightx, buttomrighty)))

    return windows

def msw_find_cars(img,
    ...
    topleftyxs = {}
    for size in range(windows_size, 64, 8):
        topleftyx = topleft(w, h, xstart, ystart, size, scale, overlap)
        #topleftyxs.update(topleftyx)
        for key in topleftyx.keys():
            if key in topleftyxs:
                topleftyxs[key] += topleftyx[key]
            else:
                topleftyxs[key] = topleftyx[key]

    #topleftyx = topleft(w, h, xstart, ystart, windows_size, scale, overlap)
    windows = mult_scale_windows(topleftyxs, overlap)
    ...
    for window in windows:
            ...
            subimg = img[top:buttom, left:right, :]
            ...
            hog_feat1 = get_hog_features(ch1, orient, pix_per_cell,
                                    cell_per_block, feature_vec=False).ravel()
            hog_feat2 = get_hog_features(ch2, orient, pix_per_cell,
                                    cell_per_block, feature_vec=False).ravel()
            hog_feat3 = get_hog_features(ch3, orient, pix_per_cell,
                                    cell_per_block, feature_vec=False).ravel()
            hog_features = np.hstack((hog_feat1, hog_feat2, hog_feat3))
            ...
            if test_prediction == 1:
                cv2.rectangle(draw_img, (left, top), (right + asize//2, buttom + asize//2),(0, 0, 255), 6)
                bbox_list.append(((left, top),(right + asize//2, buttom + asize//2)))
